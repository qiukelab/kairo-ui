...
export const Route = createFileRoute('/th/docs/$')({
  ...
  // A bad slug under `/th/docs/` (e.g. `/th/docs/no-such-page`) still
  // matches this route — unlike a totally unmatched path, there's no
  // `invariant()` throw here (see `routes/$.tsx`) — but the client's
  // dehydrated-match lookup in `ssr-client.js`'s `hydrate()` finds no
  // counterpart for it and sets `match.ssr = false`. That flips
  // `resolvedNoSsr` to `true` in `Match.js`, which forces this match through
  // a `<ClientOnly fallback={pendingElement}>` Suspense boundary. With no
  // `pendingComponent` configured, `pendingElement` is `null`, so the very
  // first client render paints nothing where the server painted the full
  // `NotFound` tree — a hydration mismatch that makes React discard the SSR
  // DOM and blank-flash before recovering. `pendingComponent: NotFound`
  // makes that first client paint match the prerendered bytes exactly, so
