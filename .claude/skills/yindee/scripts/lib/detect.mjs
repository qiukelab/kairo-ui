...
function stacksAt(dir) {
  ...
  for (const [stack, markers] of STACK_MARKERS) {
    if (
      markers.some((m) =>
        m.includes('*') ? names.some((n) => matchGlob(n, m)) : names.includes(m),
      )
    ) {
      out.push(stack)
    }
...
function detectPackageManager(root, stacks) {
  if (stacks.includes('node')) {
    ...
    const declared =
      typeof pkg.packageManager === 'string' ? pkg.packageManager.split('@')[0] : null
...
function packageAt(root, rel) {
  ...
  if (stacks.includes('node')) {
    ...
    const deployable =
      /^(apps|services)\//.test(p.path) || !!pkg.scripts?.dev || !!pkg.scripts?.start
    ...
    p.rawDeps = Array.isArray(deps)
      ? deps.map((d) => String(d).split(/[<>=!~ \[]/)[0]).filter(Boolean)
      : []
    ...
    p.rawDeps = uniq(
      [...mod.matchAll(/^\s*(?:require\s+)?([\w.\-~/]+\.[\w.\-~/]+)\s+v\d/gm)].map((m) => m[1]),
    )
  } else {
    p.stack = stack
    ...
    const names = listDir(abs)
      .filter((e) => e.isFile())
      .map((e) => e.name)
    p.manifest = names.find((n) =>
      /\.(sln|csproj|fsproj)$|^(pom\.xml|build\.gradle(\.kts)?|Gemfile|composer\.json|mix\.exs|pubspec\.yaml|Package\.swift|deno\.jsonc?)$/.test(
        n,
      ),
    )
...
function testPathsFor(root, pkg) {
  ...
  const found = candidates
    .filter((c) => exists(path.join(base, c)))
    .map((c) => (pkg.path === '.' ? c : `${pkg.path}/${c}`))
  return found
}
...
const pickScript = (scripts, key) =>
  (SCRIPT_ALIASES[key] || []).find((n) => scripts && scripts[n]) || null
  ...
  switch (stack) {
    ...
    case 'go':
      return {
        ...
        fmt: 'gofmt -w .',
        lint:
          exists(path.join(root, '.golangci.yml')) || exists(path.join(root, '.golangci.yaml'))
            ? 'golangci-lint run ./...'
            : 'go vet ./...',
        lintPkg: 'go vet ./{path}/...',
        typecheck: 'go build ./...',
    ...
    case 'python': {
      ...
      const hasRuff =
        !!tools.ruff ||
        exists(path.join(root, 'ruff.toml')) ||
        exists(path.join(root, '.ruff.toml'))
    ...
    case 'elixir':
      return {
        ...
        fmtCheck: 'mix format --check-formatted',
        fmt: 'mix format',
        lint: 'mix credo --strict',
        test: 'mix test',
        testPkg: 'mix test {path}',
      }
    case 'dart':
      return {
        install: 'dart pub get',
        fmtCheck: 'dart format --output=none --set-exit-if-changed .',
        lint: 'dart analyze',
        test: 'dart test',
        testPkg: 'dart test {path}',
      }
    case 'deno':
      return {
...
function detectCI(root) {
  ...
  if (exists(wfDir)) {
    ...
    for (const e of listDir(wfDir)) {
      ...
      if (jobsIdx !== -1) {
        for (const m of src.slice(jobsIdx).matchAll(/^ {2}([A-Za-z0-9_-]+):\s*$/gm))
          out.jobs.push(m[1])
      }
...
function frameworksOf(packages) {
  ...
  for (const [dep, label] of FRAMEWORKS) {
    if (declared.has(dep) || [...declared].some((d) => d === dep || d.endsWith(`/${dep}`)))
      hit.push(label)
  }
