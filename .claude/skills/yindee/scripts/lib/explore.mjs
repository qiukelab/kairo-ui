  ...
  if (scope.length > 1) {
    groups = scope
      .map((p) => ({
        title: p.name,
        paths: [p.path],
        sort: [depthOf(map, p.name), p.files || 0, p.name],
      }))
      .sort(
        (a, b) =>
          a.sort[0] - b.sort[0] ||
          a.sort[1] - b.sort[1] ||
          String(a.sort[2]).localeCompare(String(b.sort[2])),
      )
  } else {
    const pkg = scope[0]
    if (!pkg) return []
...
export function assessExploration(root, map, ctx, opts = {}) {
  ...
  const decomposition =
    breadth === 'broad' && !opts.skipDecomposition ? decompose(root, map, ctx, opts) : []
...
export function renderExploration(policy) {
  ...
  if (policy.level === 'none') {
    ...
    out.push(
      `        ${policy.maxAgents} agent max, sequential, scoped: ${policy.scope.slice(0, 6).join(', ') || '(repo root)'}`,
    )
  }
  ...
  if (policy.decomposition.length) {
    out.push(
      `phases  broad task -> ${policy.decomposition.length} area(s); do one per pass, verify between:`,
    )
    out.push(
      columns(
        policy.decomposition.map((p) => [`  ${p.n}.`, p.title, p.paths.slice(0, 3).join(',')]),
      ),
    )
  }
  return out.join('\n')
