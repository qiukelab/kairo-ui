...
export function resolveReference(mainRoot, spec, cwd = process.cwd()) {
  ...
  if (!found) {
    return {
      spec: raw,
      available: false,
      reason: `path not found (tried ${tried.map(toPosix).join(', ')})`,
    }
  }
...
export function queryReference(mainRoot, spec, task, mainCtx, opts = {}) {
  ...
  try {
    ...
    return {
      ...resolved,
      available: false,
      reason: `could not map reference: ${err?.message || err}`,
      files: [],
      pairs: [],
      referenceOnly: [],
    }
...
export function renderReferences(refs) {
  ...
  for (const r of refs) {
    ...
    if (r.referenceOnly.length) {
      out.push(
        '       reference-only: ' +
          r.referenceOnly.slice(0, 6).join(', ') +
          (r.referenceOnly.length > 6 ? ` … +${r.referenceOnly.length - 6}` : ''),
      )
    }
    if (!r.pairs.length && !r.referenceOnly.length)
      out.push('       no candidate matched this task in the reference')
  }
  return out.join('\n')
}
