...
export function readAgentActivity({
  ...
  if (!file) {
    return {
      status: 'unavailable',
      reason: id
        ? `no session transcript found for ${id}`
  ...
  return {
    ...
    sidechain: sideRequests.size
      ...
      : {
          status: 'unavailable',
          reason:
            'no sidechain usage records in this transcript (subagent turns may be logged elsewhere)',
        },
  }
}
...
export function estimateContextTokens(bytes) {
  return {
    label: 'estimate',
    method: 'contextCharacters/4',
    value: Math.round((Number(bytes) || 0) / 4),
  }
}
