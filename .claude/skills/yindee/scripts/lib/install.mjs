...
export function install(skillRoot, target, opts = {}) {
  ...
  if (!opts.dryRun) {
    ...
    for (const dir of ['scripts', 'rules', 'references', 'templates']) {
      if (exists(path.join(skillRoot, dir)))
        copyDir(path.join(skillRoot, dir), path.join(dest, dir))
    }
    for (const file of ['SKILL.md', 'README.md']) {
      if (exists(path.join(skillRoot, file)))
        fs.copyFileSync(path.join(skillRoot, file), path.join(dest, file))
    }
  }
  ...
  if (!exists(claudePath)) {
    ...
    const body = tmpl
      .replace(/\{\{YINDEE_BLOCK\}\}/g, block)
      .replace(/\{\{REPO\}\}/g, path.basename(target))
    ...
    if (cur.includes(START) && cur.includes(END)) {
      ...
      if (!opts.dryRun)
        fs.writeFileSync(claudePath, cur.replace(/\s*$/, '\n') + '\n' + block + '\n')
    }
  }
