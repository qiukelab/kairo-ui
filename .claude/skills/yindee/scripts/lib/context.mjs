...
export function computeContext(root, map, task, opts = {}) {
  ...
  if (!pkgs.length && map.packages.length === 1)
    pkgs = [{ pkg: map.packages[0], score: 1, why: ['only package'] }]
  ...
  if (pkgs.length && (explicitPaths.length > 0 || candidates.length < 3)) {
    candidates = fileCandidates(root, map, pkgs, tokens, areas, limits.maxCandidates, {
      fill: true,
    })
  }
  ...
  ctx.exploration = assessExploration(root, map, ctx, {
    skipDecomposition: explicitPaths.length > 0,
  })
...
export function renderContext(ctx, map, skillRoot, opts = {}) {
  ...
  const docs = []
  if (map.docs.agentInstructions.length)
    docs.push(...map.docs.agentInstructions.map((d) => `${d} (project rules)`))
  docs.push(...ctx.localInstructions.map((d) => `${d} (package rules)`))
  if (
    ctx.areas.length &&
    map.docs.architecture.length &&
    ctx.tokens.some((t) =>
      ['architecture', 'design', 'refactor', 'structure', 'migrate'].includes(t),
    )
  ) {
  ...
  if (ctx.files.length) {
    ...
    out.push(
      'files  no confident match — run: yindee context "<more specific task>" or pass --paths',
    )
  }
  ...
  if (!ctx.budget.withinBudget && ctx.budget.deferred.length) {
    out.push(
      `       next batch: yindee context "${ctx.task}" --batch ${Math.min(ctx.budget.batch + 1, ctx.budget.batches)}`,
    )
  }
  ...
  const testCmd =
    p && c.testPkg ? c.testPkg.replace('{pkg}', p.name).replace('{path}', p.path) : c.test
