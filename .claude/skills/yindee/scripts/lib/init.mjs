...
export function summarize(root, map) {
  ...
  return {
    ...
    commands: Object.fromEntries(
      ['fmtCheck', 'lint', 'typecheck', 'test', 'build']
        .filter((k) => map.commands[k])
        .map((k) => [k, map.commands[k]]),
    ),
    ci: map.ci?.provider
      ? { provider: CI_LABELS[map.ci.provider] || map.ci.provider, files: map.ci.files }
      : null,
    git: {
      isRepo: !!git.isRepo,
...
export function renderInit(res) {
  ...
  if (s.git.isRepo)
    rows.push([
      'Git',
      `${s.git.branch || 'detached'}${s.git.dirty ? ' (dirty)' : ''}${s.git.remote ? ` · ${s.git.remote}` : ''}`,
    ])
  ...
  if (!res.persisted)
    out.push(
      'note   state could not be written — this repo is read-only; init will re-run next time',
    )
  return out.join('\n')
}
