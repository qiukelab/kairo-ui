...
export function applyBudget(root, candidates, limits, { batch = 1 } = {}) {
  const sized = candidates
    ...
    .map((c) => ({ ...c, bytes: sizeOf(root, c.file) }))
  ...
  for (const c of sized) {
    const wouldExceed =
      cur.length >= limits.maxFiles || (cur.length > 0 && curBytes + c.bytes > limits.maxBytes)
    if (wouldExceed) {
...
export const fmtBudget = (b) =>
  ...
  (b.withinBudget
    ? ' · within budget'
    : ` · batch ${b.batch}/${b.batches}, ${b.deferred.length} deferred (${b.reason})`)
