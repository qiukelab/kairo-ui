...
function classify(map, files, config) {
  ...
  const manifests = files
    .filter((f) => map.manifestFiles.includes(f.path) || isLockfile(f.path))
    .map((f) => f.path)
  const codeFiles = files.filter(
    (f) => !areasOf(f.path, config).every((a) => NON_CODE_AREAS.has(a)),
  )
  ...
  if (!files.length) {
    ...
    tier = 'critical'
    reasons.push(
      `sensitive paths: ${uniq(sensitive.slice(0, 4)).join(', ')}${sensitive.length > 4 ? ` (+${sensitive.length - 4})` : ''}`,
    )
  }
...
function planFor(map, tier, pkgs, base) {
  ...
  if (scoped) {
    for (const p of pkgs) {
      ...
      if (c.typecheckPkg)
        push(`typecheck:${p.name}`, fill(c.typecheckPkg, vars), `typecheck ${p.name}`)
      else if (c.typecheck && !c.typecheckPkg)
        push('typecheck', c.typecheck, 'no per-package typecheck available')
...
export function computeImpact(root, map, opts = {}) {
  ...
  const files = (
    opts.paths?.length ? opts.paths.map((p) => ({ path: p, status: 'M' })) : changed.files
  ).filter((f) => opts.includeGenerated || !isGenerated(f.path) || isLockfile(f.path))
...
export function renderImpact(impact, { verbose = false } = {}) {
  ...
  if (impact.packages.direct.length) {
    ...
    out.push(
      `pkgs   ${impact.packages.direct.join(', ')}` +
        (extra.length ? `  +dependents: ${extra.join(', ')}` : ''),
    )
  }
  if (impact.sensitive.length)
    out.push(`!!     sensitive: ${uniq(impact.sensitive).slice(0, 8).join(', ')}`)

  const show = verbose ? impact.files : impact.files.slice(0, 25)
  if (show.length) {
    out.push('files')
    out.push(
      columns(show.map((f) => ['  ' + f.status, f.path, areasOf(f.path).slice(0, 2).join(',')])),
    )
    if (impact.files.length > show.length)
      out.push(`  … ${impact.files.length - show.length} more (--verbose)`)
  }
