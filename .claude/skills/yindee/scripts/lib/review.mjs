...
const BINARY_RE =
  /\.(png|jpe?g|gif|webp|ico|svgz|woff2?|ttf|eot|pdf|zip|gz|tar|mp4|mp3|wasm|onnx|bin|so|dll|dylib|exe|class|jar)$/i
...
function untrackedBlocks(root, files, maxPerFile) {
  ...
  for (const f of files) {
    ...
    if (BINARY_RE.test(f.path)) {
      out.push({
        file: f.path,
        lines: [`diff --git a/${f.path} b/${f.path}`, 'new binary file (content not shown)'],
      })
      continue
    }
    ...
    out.push({
      ...
      lines: [
        ...
        ...(body.length > shown.length
          ? [`... (${body.length - shown.length} more lines in ${f.path})`]
          : []),
      ],
    })
  }
...
export function buildReview(root, map, impact, opts = {}) {
  ...
  for (const p of parts) {
    ...
    if (text.length > maxPerFile) {
      text = [
        ...
        `... (${p.lines.length - maxPerFile} more lines in ${p.file})`,
      ]
    }
    total += text.length
  ...
  const failures = (verify?.results || []).filter(
...
export function renderReview(review, impact, map, skillRoot) {
  const out = []
  out.push(
    `review ${impact.tier.toUpperCase()} change · depth:${impact.reviewDepth} · ${impact.files.length} file(s)` +
  ...
  if (map.docs.agentInstructions.length)
    out.push('project ' + map.docs.agentInstructions.join(', ') + ' also apply')
  ...
  for (const [a, files] of Object.entries(byArea)) {
    out.push(
      `  ${a}: ${files.slice(0, 8).join(', ')}${files.length > 8 ? ` (+${files.length - 8})` : ''}`,
    )
  }
  if (review.stat) {
    out.push('stat')
  ...
  if (review.omitted.length) {
    out.push('')
    out.push(
      `omitted ${review.omitted.length}: ${review.omitted.slice(0, 10).join(', ')}${review.omitted.length > 10 ? ' …' : ''}`,
    )
