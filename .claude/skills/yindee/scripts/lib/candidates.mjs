...
function grepCandidates(root, tokens, scopePaths) {
  ...
  const ranked = [...tokens]
    .filter((t) => t.length >= 4 && /^[a-z0-9]+$/.test(t))
    .sort((a, b) => b.length - a.length)
  for (const tok of ranked.slice(0, 5)) {
    const found = lines(`git grep -l -I -i -F -e "${tok}"${scope}`, {
      cwd: root,
      timeout: 20_000,
    }).slice(0, 25)
...
const isSource = (rel) =>
  !isGenerated(rel) && !NON_CODE_AREAS.has(primaryArea(rel)) && primaryArea(rel) !== 'config'
  ...
  for (const r of roots) {
    for (const f of collectFiles(root, r, { maxDepth: 6, limit: 3000 })) {
      ...
      if (
        ...
        matchAny(
          f,
          areas.flatMap((a) => DEFAULT_AREAS[a] || []),
        )
      )
        bump(f, 1, 'area')
