...
// headline is not.
const HEADLINE_RE =
  /(^\s*not ok\b|^\s*FAIL\b|^\s*✕|^\s*×|\bAssertionError\b|\bpanicked at\b|^\s*error(\[[A-Z]\d+\])?:|^\s*ERROR\b|^.+\.[a-z]+:\d+:\d+[ :]|\bTS\d{4}\b)/
const DETAIL_RE =
...
export function columns(rows, gap = 2) {
  ...
  return rows
    .map((r) =>
      r
        .map((cell, i) =>
          i === r.length - 1 ? String(cell ?? '') : String(cell ?? '').padEnd(widths[i]),
        )
        .join(' '.repeat(gap))
        .replace(/\s+$/, ''),
    )
...
const STOP = new Set(
  (
    ...
    'need want should must can code file files feature bug issue task work them their our'
  ).split(' '),
)
