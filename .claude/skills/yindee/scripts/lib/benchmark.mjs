...
export function renderReport(run) {
  ...
  const head = [
    [
      'Duration',
      fmtDuration(run.wallClockDurationMs) + (run.status === 'running' ? ' (in progress)' : ''),
    ],
  ...
  section(out, 'Context', [
    ...
    [
      'Estimated tokens',
      `~${fmtCount(est.value ?? 0)} (estimate only — Yindee output, not Claude usage)`,
    ],
    ...
    ...(run.reference
      ? [
          ...
          ...(run.reference.unavailable
            ? [['Reference unavailable', String(run.reference.unavailable)]]
            : []),
        ]
      : []),
  ])
  ...
  section(
    ...
    ag.status === 'ok'
      ? [
          ...
          ...(ag.sidechain?.status === 'ok'
            ? []
            : [['  reason', ag.sidechain?.reason || 'not recorded']]),
          ['Source', ag.source],
        ]
      : [
...
const fmtDelta = (d, format = String) =>
  ...
  (d.percent === null ? '(n/a from zero)' : `${d.percent > 0 ? '+' : ''}${d.percent.toFixed(1)}%`)
...
export function compareRuns(a, b) {
  const cmp = {
    ...
    estimatedYindeeContextTokens: {
      label: 'estimate',
      ...delta(
        a.context.estimatedYindeeContextTokens?.value,
        b.context.estimatedYindeeContextTokens?.value,
      ),
    },
    ...
    broadExplorationRecommended: delta(
      a.exploration?.broadRecommended,
      b.exploration?.broadRecommended,
    ),
    verification: {
  ...
  if (a.changes?.status === 'ok' && b.changes?.status === 'ok') {
    ...
    cmp.changes = {
      status: 'unavailable',
      reason: 'change measurement missing for at least one run',
    }
  }
  ...
  cmp.agents =
    aa.status === 'ok' && ab.status === 'ok'
      ? {
          status: 'ok',
          spawned: delta(aa.spawned, ab.spawned),
          exploration: delta(aa.exploration, ab.exploration),
        }
  ...
  if (ta.status === 'ok' && tb.status === 'ok') {
    ...
    cmp.claudeTokenUsage = {
      status: 'unavailable',
      reason:
        `actual token usage missing for ${ta.status === 'ok' ? cmp.b.id : cmp.a.id}` +
  ...
  out.push(
    columns(
      [
        ...
        [
          '  est. tokens',
          fmtDelta(cmp.estimatedYindeeContextTokens, fmtCount) + '  (estimate vs estimate)',
        ],
  ...
  out.push(
    columns(
      [
        ...
        [
          '  subagents spawned',
          cmp.agents.status === 'ok'
            ? fmtDelta(cmp.agents.spawned)
            : `${UNAVAILABLE} — ${cmp.agents.reason}`,
        ],
        [
          '  exploration agents',
          cmp.agents.status === 'ok' ? fmtDelta(cmp.agents.exploration) : UNAVAILABLE,
        ],
      ],
      1,
    ),
