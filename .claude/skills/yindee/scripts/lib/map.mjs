...
export function loadMap(root, { force = false, write = true } = {}) {
  ...
  if (!force) {
    ...
    if (
      cached &&
      cached.mapVersion === MAP_VERSION &&
      cached.harness === harnessFingerprint() &&
      cached.manifestFiles
    ) {
...
export function renderMap(map, { verbose = false } = {}) {
  ...
  out.push(
    `repo   ${map.root}` +
      (g.isRepo
        ? `  git:${g.branch}${g.base && g.base !== g.branch ? ` (base ${g.base})` : ''}`
        : '  (no git)') +
  ...
  if (map.packages.length) {
    ...
    if (map.packages.length > 40 && !verbose)
      out.push(`  … ${map.packages.length - 40} more (--verbose)`)
  }
  ...
  const scoped = map.monorepo.isMonorepo
    ? ['lintPkg', 'typecheckPkg', 'testPkg'].filter((k) => map.commands[k])
    : []
  if (scoped.length)
    out.push('scoped ' + scoped.map((k) => `${k}: ${map.commands[k]}`).join('\n       '))
  if (map.affected)
    out.push(
      'affect ' +
        Object.entries(map.affected)
          .map(([k, v]) => `${k}: ${v}`)
          .join('\n       '),
    )
  ...
  if (map.ci.provider) {
    out.push(
      `ci     ${map.ci.provider}: ${map.ci.files.map((f) => f.split('/').pop()).join(', ')}` +
        (map.ci.jobs.length ? ` [${map.ci.jobs.join(', ')}]` : ''),
    )
  }
