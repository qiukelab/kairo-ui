...
function main() {
  switch (cmd) {
    ...
    case 'verify': {
      ...
      if (flags.ci) {
        ...
        if (!steps.length) {
          return emit(
            { steps: [] },
            `verify --ci: no runnable commands found in ${map.ci.provider || 'CI config'}` +
      ...
      if (flags['dry-run']) {
        return emit(
          { tier: impact.tier, steps },
          `verify [${impact.tier}] dry run:\n` + columns(steps.map((s) => ['  ' + s.id, s.cmd])),
        )
      }
      ...
      measured.failures = results.filter(
      ...
      saveResults(ROOT, {
        tier: impact.tier,
        base: impact.base,
        files: impact.files.map((f) => f.path),
        results,
        totalMs,
      })
      if (results.some((r) => r.status === 'fail' || r.status === 'timeout')) process.exitCode = 1
      return emit(
        { tier: impact.tier, results, totalMs },
        renderResults(results, { tier: impact.tier, totalMs }),
      )
    }
    ...
    case 'status': {
      ...
      out.push(
        `branch ${st.branch}${st.base && st.base !== st.branch ? ` (base ${st.base})` : ''}` +
          (st.aheadBehind
            ? `  ahead ${st.aheadBehind.ahead}/behind ${st.aheadBehind.behind} of ${st.aheadBehind.upstream}`
            : '  no upstream') +
          (st.worktrees > 1 ? `  · ${st.worktrees} worktrees` : ''),
      )
      out.push(
        `work   ${impact.files.length} changed file(s) · tier ${impact.tier}${st.dirty ? ' · dirty' : ''}`,
      )
      if (gh.pr) {
        ...
        out.push(
          `pr     #${gh.pr.number} ${gh.pr.draft ? '(draft) ' : ''}${gh.pr.title}` +
            (c.total
              ? `  checks ${c.total - c.failing - c.pending}/${c.total} ok${c.failing ? `, ${c.failing} FAILING` : ''}${c.pending ? `, ${c.pending} pending` : ''}`
              : ''),
        )
      } else if (gh.authed) {
      ...
      if (gh.issue) {
        out.push(
          `issue  #${gh.issue.number} [${gh.issue.state}] ${gh.issue.title}` +
            (gh.issue.milestone ? `  milestone: ${gh.issue.milestone}` : ''),
        )
      }
      ...
      if (last) {
        ...
        out.push(
          `verify last run [${last.tier}] ${bad.length ? `${bad.length} failing: ${bad.map((r) => r.id).join(', ')}` : 'all green'} (${fmtMs(last.totalMs || 0)})`,
        )
      }
      return emit(
        { git: st, gh, impact: { tier: impact.tier, files: impact.files.length } },
        out.join('\n'),
      )
    }
    ...
    case 'install': {
      ...
      return emit(
        res,
        `install ${flags['dry-run'] ? '(dry run) ' : ''}-> ${target}\n` +
    ...
    case 'doctor': {
      ...
      rows.push([
        'stacks',
        map.stacks.join('+') || 'none detected',
        map.stacks.length ? 'ok' : 'CHECK',
      ])
      rows.push(['packages', String(map.packages.length), map.packages.length ? 'ok' : 'CHECK'])
      rows.push([
        'map cache',
        toPosix(path.relative(ROOT, mapPath(ROOT))),
        cached ? 'hit' : 'rebuilt',
      ])
      rows.push([
        'initialized',
        isInitialized(ROOT) ? 'yes' : 'no',
        measured.initStatus || 'unknown',
      ])
      rows.push([
        'frameworks',
        map.frameworks?.join(', ') || '(none declared)',
      ...
      for (const key of ['fmtCheck', 'lint', 'typecheck', 'test', 'build']) {
        ...
        rows.push([
          `cmd ${key}`,
          c || '(none detected)',
          c ? (have(c.trim().split(/\s+/)[0]) ? 'ok' : 'tool missing') : '-',
        ])
      }
      rows.push([
        'ci',
        map.ci.provider || '(none)',
        map.ci.files.length ? `${map.ci.files.length} file(s)` : '-',
      ])
      ...
      rows.push([
        'rules',
        missing.length ? `missing: ${missing.join(', ')}` : 'frontend backend database security',
        missing.length ? 'CHECK' : 'ok',
      ])
  ...
  switch (sub) {
    ...
    case 'status': {
      ...
      if (st.corrupt)
        return emit(
          st,
          `benchmark session state is corrupt (${st.file}) — run: yindee benchmark stop`,
        )
      if (!st.active) {
        const ids = tel.listRuns(ROOT)
        return emit(
          { active: false, runs: ids },
          `benchmark no session running` +
            (ids.length
      ...
      return emit(
        st,
        `benchmark ${sm.id} running for ${fmtDuration(sm.wallClockDurationMs)}` +
    ...
    case 'report': {
      ...
      if (!run) {
        ...
        return emit(
          { error: 'no such run' },
          `benchmark no run matches "${ref || 'latest'}" (yindee benchmark list)`,
        )
      }
      if (run.corrupt) {
        process.exitCode = 1
        return emit(
          { error: 'corrupt run', id: run.id },
          `benchmark run ${run.id} is unreadable — its file is corrupt`,
        )
      }
    ...
    case 'compare': {
      ...
      if (!refA || !refB) {
        process.exitCode = 2
        return emit(
          { error: 'two runs required' },
          'usage: yindee benchmark compare <run-a> <run-b>',
        )
      }
      ...
      if (bad.length) {
        process.exitCode = 2
        return emit(
          { error: 'unreadable run(s)', refs: bad },
          `benchmark cannot read run(s): ${bad.join(', ')}`,
        )
      }
    ...
    case 'list': {
      ...
      const rows = ids.map((id) => {
        ...
        return [
          '  ' + id,
          fmtDuration(r.wallClockDurationMs),
          `${r.commands.total} cmd`,
          r.label || '',
        ]
      })
    ...
    case 'prune': {
      const res = tel.pruneRuns(
        ROOT,
        flags.keep !== undefined ? Number(flags.keep) : tel.DEFAULT_KEEP,
      )
      return emit(
        res,
        `benchmark kept ${res.kept} run(s), removed ${res.removed.length}` +
